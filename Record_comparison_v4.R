...
flood_reg1.lm = lm(log(peak_flow+0.01) ~ wy_order[[k]]) # Add constant to avoid problems with ephemeral streams
...
#plot(wy_order[[k]],(res_reg1[[k]]^2)^(1/3),xlab="Water year in record (t)",ylab= "Residual Variance ^ (2/3)")
#title(c(site_names[station_group[k,1]],
#     as.character(site_id[station_group[k,1]]),
#     "DA",
#     as.numeric(as.character(site_info[station_group[k,1],6]))),cex.main=0.8)
#text(min(wy_order[[k]])+5,max((res_reg1[[k]]^2)^(1/3))-0.08,bquote(~R^2 ==.(round(rsquared_5a[k],3))),cex=0.7)
...
int <- (length(wy_order[[k]])-1)/length(wy_order[[k]])*var(as.numeric(res_reg1[[k]]))
# find optimal cc1 value
cc1_min_RSS <- function(cc1) {
  sum((res_reg1_2 - (int + cc1*(wy_order[[k]]-mean(wy_order[[k]]))))^2)
}
...
cond_var_3a <- int + cc1_3a[k]*(wy_order[[k]]-mean(wy_order[[k]]))
...
# find optimal cc1 value
cc1_min_RSS <- function(cc1) {
  sum((res_reg1_2 - (int*exp(cc1*(wy_order[[k]]-mean(wy_order[[k]])))))^2)
}
...
# Compute conditional variance and quantile
cond_var_3c <- int + cc1_3c[k]*(wy_order[[k]]-mean(wy_order[[k]]))
...
cc <- c(0.5,0.01) 
min.RSS <- function(cc) {
  sum((res_reg1_2_3 - (b1_reg1[k]^2*sigma_t_2*rho_reg1[k])/(cc[1]+cc[2]*mean(wy_order[[k]]))*(cc[1]+cc[2]*wy_order[[k]]))^2)
}
optim(cc,min.RSS)
tot.RSS <- sum((res_reg1_2_3 - mean(res_reg1_2_3))^2)
 ...
 # Set initial values of c0 and c1
  cc_param_est <- function(cc){
    cc0 <- cc[1]
    cc1 <- cc[2]
    cond_var_res_reg2_4a_init <- (b1_reg1[k]^2*sigma_t_2*rho_reg1[k])/(cc[1]+cc[2]*mean(wy_order[[k]]))*(cc[1]+cc[2]*wy_order[[k]]) 
...
cond_var_res_reg2_4a_init <- (b1_reg1[k]^2*sigma_t_2*rho_reg1[k])/(cc0_4a[k]+cc1_4a[2]*mean(wy_order[[k]]))*wy_order[[k]] 
cond_var_4a_2_3_init <- cond_var_res_reg2_4a_init^(1/3)
...
#                                     (cov(wy_order[[k]],res_reg1_2)/var(wy_order[[k]]) * (wy_order_max[k] - wy_order_mean[k])))
...
p_cc1_5a[k] = 2*(pt(-abs(t_cc1_5a), df=length(wy_order[[k]]) - 1)) 
...
#cond_var_at_n_5b[k] = (cc0_5b[k] + cc1_5b[k] * max(wy_order[[k]]^2))^3
...
#cond_var_at_n_5c[k] = cc0_5c[k] + cc1_5c[k] * max(wy_order[[k]])^3 # ADD BIAS CORRECTION 
...
cond_var_at_n_5d[k] = res_dof_corr[k]*(cc0_5d[k] + cc1_5d[k] * mean(log(wy_order[[k]])))^3 + res_reg2_5d_var*(3*cc0_5d[k] + 3*cc1_5d[k] * mean(log(wy_order[[k]]))) + mean((res_reg1_2_3-res_reg2_5d_fit)^3) +
  cov(log(wy_order[[k]]),res_reg1_2)/var(log(wy_order[[k]])) * (max(log(wy_order[[k]])) - mean(log(wy_order[[k]])))
#plot(wy_order,res_reg1_2_3,col="blue")
...
cond_var_at_n_5e[k] = exp(res_dof_corr[k]*((cc0_5e[k] + cc1_5e[k] * max(wy_order[[k]]) + 0.5*res_reg2_5e_var)
                                           + cov(wy_order[[k]],log_res_reg1_2)/var(wy_order[[k]])*(max(wy_order[[k]])-mean(wy_order[[k]]))))  
...
cond_var_at_n_6a[k] = res_dof_corr[k]*(cc0_6a[k,10] + cc1_6a[k,10] * mean(wy_order[[k]]))^3 + 3*res_reg2_6a_var*(cc0_6a[k,10] + cc1_6a[k,10] * mean(wy_order[[k]])) + mean((res_reg1_2_3-cond_var_6a[,10])^3) + 
  cov(wy_order[[k]],res_reg1_iwls_6a[,10]^2)/var(wy_order[[k]]) * (max(wy_order[[k]]) - mean(wy_order[[k]])) 
...
cond_var_at_n_6d[k] = (cc0_6d[k,10] + cc1_6d[k,10] * mean(log(wy_order[[k]])))^3 + res_reg2_6d_var*(3*cc0_6d[k,10] + 3*cc1_6d[k,10] * mean(log(wy_order[[k]]))) + mean((res_reg1_iwls_2_3_6d[,10]-cond_var_6d[,10])^3) +
  cov(log(wy_order[[k]]),res_reg1_iwls_6d[,10]^2)/var(log(wy_order[[k]])) * (max(log(wy_order[[k]])) - mean(log(wy_order[[k]])))
...
boxplot(rsquared_5a,
        ...
        rsquared_5d,
        #rsquared_5e,
        rsquared_iwls_glm_gamma,names=c("Linear","Quadratic","Logarithmic","GLM-IWLS"),cex.axis=0.8)
